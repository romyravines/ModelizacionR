...
```{r Circulos concentricos, echo=FALSE,  fig.height=5}
set.seed(666)
x <- seq(-2, 2,  by = 0.005)
...
a8 <- ggplot(data = datos, aes(x = X, y = Y)) + 
...
```{r circulos concentricos k-medias, echo=FALSE,  fig.height=3}
cl11 <- kmeans(datos, centers = 2)
...
a5 <- ggplot(data = clusters, aes(x = k, y = SSE)) + 
  geom_point(size = 5, color = 'steelblue') + 
  geom_line(size = 2, color = 'steelblue') + 
  scale_x_continuous(breaks = 1:15) +
  ggtitle('SSE para distintos k grupos')

grid.arrange(a5, ncol = 1)
...
Uno de los algoritmos de clustering que han experimentado un mayor crecimiento en su uso es la técnica **D**ensity-**b**ased **s**patial **c**lustering of **a**pplications with **n**oise (**DBSCAN**).

Se trata de un **tipo de clustering basado en densidades**.

```{r  out.width = "75%", echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=FALSE}
img_path =paste(imgs_path,"dbscan.png",sep="")
include_graphics(img_path) 
```

El algoritmo considera los siguientes **tipos de puntos**:

 - Un punto es un **punto núcleo** (*core point*) si tiene al menos $minPts$ puntos en un radio $\epsilon$ (incluyéndose a sí mismo).

 - Un punto es **directamente alcanzable** (*directly reachable*) desde un punto núcleo si ese punto núcleo está en un radio $\epsilon$.

 - Un punto $q$ es **alcanzable** desde un punto núcleo $p$ si existe un camino $p_1, \ldots, p_n$ tal que $p_1 = p$, $p_n = q$ donde cada $p_{i+1}$ es directamente alcanzable desde $p_i$.

 - Todo punto no alzanzable por otro punto es considerado **outlier**.

Así, si $p$ es un punto núcleo, forma un cluster con todos los puntos que son alcanzables desde él.

Como propiedades, cada cluster contiene al menos un punto núcleo y, los puntos que no son de tipo núcleo, se considerar como el *borde* del cluster.


Las **ventajas** de esta técnica son:

 - No es necesario especificar a priori el número de clusters (al contrario que en las k-medias).
 - La técnica puede encontrar clusters con formas arbitrarias. 
 - Es robusto frente a outliers
 - Los parámetros del algoritmo $minPts$ y $\epsilon$ se pueden elegir por el analista si se entiende bien el problema para el que se quiere aplicar.

Las **desventajas** serían:

 - no es enteramente determinístico, es decir, puntos que están en el borde de varios clusters, puede depender pertencer a uno u a otro en función del orden en el que se procesen los datos.
 - la calidad de la solución depende de la medida usada para definir la vecindad de radio $epsilon$. La más usada suele ser la distancia euclídea.
